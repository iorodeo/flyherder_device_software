...
int AccelStepper::desiredSpeed()
{
    ...
    long sqrSpeed = ((long)_speed)*((long)_speed);
    if (_speed < 0)
      sqrSpeed = -sqrSpeed;
    unsigned int twoa = 2*_acceleration; // 2ag
    ...
    if ((sqrSpeed / twoa) < distanceTo) {
        ...
        if (_speed == 0) {
            requiredSpeed = fastRoot(twoa);
        }
        else {
            requiredSpeed = (int) ((((long)_speed)*((long)absSpeed) + (long)_acceleration)/absSpeed);
        }
...
void AccelStepper::setMaxSpeed(unsigned int speed)
{
    _maxSpeed = speed;
    computeNewSpeed();
}

void AccelStepper::setAcceleration(unsigned int acceleration)
{
    _acceleration = acceleration;
    computeNewSpeed();
}

void AccelStepper::setSpeed(int speed)
{
    if (speed == _speed) {
        return;
    }
    ...
    _stepInterval = labs(1000000L/((long)_speed));
}

int AccelStepper::speed()
{
    return _speed;
}
...
//
// From IAR Application Note G-002, Fast square root in C
// 
